...
`;
    ...
    static async autoParser(expr, attributes, itemattributes, exprmode, noreg = false, number = 1, uses = 0) {
        ...
        while (!sums_are_num) {
            ...
            //LISTCONTAINS
            // listcontains(a,b,c,@{someprop.listoptions};hello)
            let listcontainsmatch = /\blistcontains\(/g;
            var listcontainsResultArray;
            var listcontainsResult = [];

            while (listcontainsResultArray = listcontainsmatch.exec(expr)) {
                let suba = expr.substring(listcontainsmatch.lastIndex, expr.length);
                let subb = auxMeth.getParenthesString(suba);
                listcontainsResult.push(subb);
            }

            if (listcontainsResult != null) {
                //Substitute string for current value
                for (let i = 0; i < listcontainsResult.length; i++) {
                    let splitter = listcontainsResult[i].split(";");
                    let comparer = listcontainsResult[i].replace(splitter[0] + ";", '');
                    let blocks = splitter[0].split(",");

                    let tochange = "listcontains(" + listcontainsResult[i] + ")";
                    expr = expr.replace(tochange, blocks.includes(comparer) ? "true" : "false");
                }
            }
